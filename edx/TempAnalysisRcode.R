...
#data <- read.table("../data/AvTempAtlanta.txt",header=T)
data <- read.csv("../data/or_monthly_mean_temp_1895-2019.csv")
head(data)
temp <- ts(data$Value, start=1895, frequency=12)
...
temp.fit.mav <- ts(mav.fit$y, start=1895, frequency=12)
ts.plot(temp, ylab="Temperature")
...
summary(lm.fit)
temp.fit.lm <- ts(fitted(lm.fit), start=1895, frequency=12)
ts.plot(temp, ylab="Temperature")
...
temp.fit.loc <- ts(fitted(loc.fit), start=1895, frequency=12)
## Splines Trend Estimation
gam.fit <- gam(temp ~ s(time.pts))
temp.fit.gam <- ts(fitted(gam.fit), start=1895, frequency=12)
## Is there a trend? 
ts.plot(temp, ylab="Temperature")
...
summary(lm.fit)
dif.fit.lm <- ts((temp - fitted(lm.fit)), start=1895, frequency=12)
...
dif.fit.gam <- ts((temp - fitted(gam.fit)), start=1895, frequency=12)
